...
func init() {
	...
	h.ChangeProductionQty().AddFields(map[string]models.FieldDefinition{
	...
	h.ChangeProductionQty().Methods().DefaultGet().Extend(
		`DefaultGet`,
		func(rs m.ChangeProductionQtySet, fields interface{}) {
	...
	h.ChangeProductionQty().Methods().UpdateProductToProduce().DeclareMethod(
		...
		func(rs m.ChangeProductionQtySet, production interface{}, qty interface{}) {
			...
			//            production_move = production.move_finished_ids.filtered(lambda x: x.state not in (
			//                'done', 'cancel') and production.product_id.id == x.product_id.id)
			//            production_move.write({'product_uom_qty': qty})
		})
	h.ChangeProductionQty().Methods().ChangeProdQty().DeclareMethod(
		`ChangeProdQty`,
		func(rs m.ChangeProductionQtySet) {
			//        precision = self.env['decimal.precision'].precision_get(
			//            'Product Unit of Measure')
			//        for wizard in self:
			//            production = wizard.mo_id
			//            produced = sum(
			//                production.move_finished_ids.mapped('quantity_done'))
			//            if wizard.product_qty < produced:
			//                raise UserError(_("You have already processed %d. Please input a quantity higher than %d ") % (
			//                    produced, produced))
